...
const HeaderHeight = 60; // 헤더 높이 보정값
...
const ScrollSpy = () => {
  ...
  const navsRef = useRef<(HTMLElement | null)[]>([]);
  ...
  const handleNavClick = useCallback((index: number) => {
    ...
    // elem.scrollIntoView({ behavior: 'smooth' }); // 해당 elem가 보여지는 위치로 이동(좌표계산은 브라우저가 알아서 함)
    window.scrollTo({
      top: itemY, // 픽셀 해당 좌표로 이동
      behavior: 'smooth',
    });
  }, []);
    ...
    <div className={cx('ScrollSpy')}>
      <header className={cx('floatingHeader')}>
        <h3 className={cx('title')}>
          Scroll Spy #1 <sub>scroll event</sub>
        </h3>
        <ul className={cx('nav')}>
          {data.map(({ index, id }) => (
            <li
